...
class ProbingClassifier(models.Model):
    def __init__(self,
        ...
        self._classilayer = layers.Dense(units = classes_num)
        # TODO(students): start
        # ...
        # TODO(students): end
    ...
    def call(self, inputs: tf.Tensor, training: bool =False) -> tf.Tensor:
        ...
        """
        # TODO(students): start
        op = self._pretrained_model(inputs)

        rep = tf.gather(op["layer_representations"], self._layer_num - 1, axis = 1)
        logits = self._classilayer(rep) 
